...
export default function ReceptionistDashboard() {
  ...
  const waiting = mockAppointments.filter(a => a.status === 'Waiting')
  const confirmed = mockAppointments.filter(a => a.status === 'Confirmed')
  const emergency = mockAppointments.filter(a => a.priority === 'Emergency')

  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-black text-gray-900">Reception Dashboard</h1>
        <p className="text-gray-500 text-sm mt-0.5">Welcome, {user?.name} 👋 — Managing front desk operations</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Today', value: mockAppointments.length, icon: Calendar, bg: 'bg-blue-50', color: 'text-blue-600' },
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="dashboard-card">
        ...
        <div className="space-y-3">
          {mockAppointments.slice(0, 6).map((a, idx) => (
            <div key={a.id} className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl">
              ...
              <span className={`badge ${a.status === 'Waiting' ? 'badge-warning' : a.status === 'In Progress' ? 'badge-info' : a.status === 'Confirmed' ? 'badge-success' : 'badge-danger'}`}>{a.status}</span>
            </div>
          ))}
        </div>
      </motion.div>
