...
1. deploy  - Pulls the chosen branch, builds and starts Docker containers with dynamic conflict-free ports.
                ...
                script {
                    ...
                    switch(params.ACTION) {
                        ...
                        case 'down':
                            ...
                            sh "export COMPOSE_PROJECT_NAME=medysync && ${dockerCmd} down --remove-orphans"
                            break

                        case 'restart':
                            echo '🔄 Restarting running Docker containers...'
                            sh "export COMPOSE_PROJECT_NAME=medysync && ${dockerCmd} restart"
                            break

                        case 'logs':
                            echo '📋 Fetching container logs...'
                            sh "export COMPOSE_PROJECT_NAME=medysync && ${dockerCmd} logs --tail=100"
                            break
                    }
                }
            }
